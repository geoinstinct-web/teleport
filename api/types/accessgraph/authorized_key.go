...
func ValidateAuthorizedKey(k *accessgraphv1pb.AuthorizedKey) error {
	...
	if k.Metadata.Name != hashNameKey(k.Spec) {
...
func hashNameKey(k *accessgraphv1pb.AuthorizedKeySpec) string {
	h := sha256.New()
	h.Write([]byte(k.HostId))
	h.Write([]byte(k.HostUser))
	h.Write([]byte(k.KeyFingerprint))
